...
(function() {
  var addUnitToRaidFrame, classComplement, classSpecsHandler, classes, complement, defaultUnitFrame, findNextEmptySlot, groupResetHandler, groups, i, invite, inviteHandler, raidResetHandler, resetInviter, resetInviterHandler, roleHandler, showRoles, specComplement, specRolesHandler, specs, _i;
  ...
  groupResetHandler = function(group) {
    return $("." + group + " .groupName a.reset").click(function(event) {
      var klass, _j, _len, _results;
      event.preventDefault();
      _results = [];
      for (_j = 0, _len = classes.length; _j < _len; _j++) {
        klass = classes[_j];
        if ($("." + group + " div").hasClass(klass)) {
          _results.push($("." + group + " div").removeClass(klass).find('span').removeClass().addClass("frame", "slot").text(""));
        } else {
  ...
  raidResetHandler = function() {
    return $(".stats .raidReset a.reset").click(function(event) {
      var group, klass, _j, _len, _results;
      event.preventDefault();
      _results = [];
      for (_j = 0, _len = groups.length; _j < _len; _j++) {
        group = groups[_j];
        _results.push((function() {
          ...
          for (_k = 0, _len1 = classes.length; _k < _len1; _k++) {
            klass = classes[_k];
            if ($("." + group + " div").hasClass(klass)) {
              _results1.push($("." + group + " div").removeClass(klass).find('span').removeClass().addClass("frame", "slot").text(""));
            } else {
  ...
  $(function() {
    var group, klass, role, spec, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _n, _ref, _ref1;
    for (_j = 0, _len = classes.length; _j < _len; _j++) {
      klass = classes[_j];
    ...
    resetInviterHandler();
    inviteHandler();
    for (_n = 0, _len4 = groups.length; _n < _len4; _n++) {
      group = groups[_n];
      groupResetHandler(group);
    }
    return raidResetHandler();
  });
